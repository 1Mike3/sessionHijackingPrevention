...
public class FpRequestProcessor {
    ...
    public static FingerprintData processRequestFp(Context ctx) {
        ...
        //Reacting to cookies not being accepted
        ...
        accept = accept == null ? "unknown" : accept.length() > 250 ? accept.substring(0, 250) : accept;
        builder.accept(accept);
        String encoding = ctx.header("Accept-Encoding");
        logger.trace("~Encoding Received: " + encoding);
        encoding = encoding == null ? "unknown" : encoding.length() > 250 ? encoding.substring(0, 250) : encoding;
